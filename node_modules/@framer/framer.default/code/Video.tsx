...
import { ControlType, RenderTarget, addPropertyControls, FrameProps } from "framer"
...
import { isBrowserSafari } from "./lib/isBrowserSafari"
...
export function Video(props) {
    const { width, height, topLeft, topRight, bottomRight, bottomLeft, id, ...rest } = props
    return <VideoMemo {...rest} />
}
...
const VideoMemo = React.memo(function VideoInner(props: VideoProps) {
    ...
    return (
        <video
            ...
            onLoadedData={() => {
                if (isCanvas && !canvasPlay && videoRef.current) videoRef.current.pause()
                setProgress(startTime)
            }}
            onCanPlay={() => {
                if (shouldPlay && videoRef.current && autoPlay) videoRef.current.play()
            }}
            src={isForcedCache || isAutoCache ? null : getUrl()}
